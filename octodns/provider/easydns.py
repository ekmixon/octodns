...
class EasyDNSProvider(BaseProvider):
    ...
    def _data_for_CNAME(self, _type, records):
        record = records[0]
        return {'ttl': record['ttl'], 'type': _type, 'value': f"{record['rdata']}"}

    def _data_for_MX(self, _type, records):
        values = [
            {'preference': record['prio'], 'exchange': f"{record['rdata']}"}
            for record in records
        ]
    ...
    def _params_for_NAPTR(self, record):
        for value in record.values:
            content = f'{value.order} {value.preference} "{value.flags}" "{value.service}" "{value.regexp}" {value.replacement}'
    ...
    def _params_for_SRV(self, record):
        for value in record.values:
            yield {
                'rdata': f"{value.priority} {value.port} {value.weight} {value.target}",
                'name': record.name,
                'ttl': record.ttl,
                'type': record._type,
            }
