...
class GandiClient(object):
    ...
    def zone_records(self, zone_name):
        records = self._request('GET', f'/livedns/domains/{zone_name}/records').json()
    ...
    def record_create(self, zone_name, data):
        self._request('POST', f'/livedns/domains/{zone_name}/records', data=data)

    def record_delete(self, zone_name, record_name, record_type):
        self._request(
            'DELETE',
            f'/livedns/domains/{zone_name}/records/{record_name}/{record_type}',
        )
    ...
    def populate(self, zone, target=False, lenient=False):
        ...
        for record in self.zone_records(zone):
            ...
            if _type not in self.SUPPORTS:
                continue
            values[record['rrset_name']][_type].append(record)
